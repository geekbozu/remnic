...
- `~/.pi/agent/extensions/remnic/remnic.config.json` — private daemon URL, session key prefix, and auth token (`0600`)
...
To target a non-default daemon or session key prefix:

```bash
remnic connectors install pi \
  --config remnicDaemonUrl=http://127.0.0.1:4318 \
  --config sessionKeyPrefix=myuser
```
