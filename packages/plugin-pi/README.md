...
The generated `remnic.config.json` contains the Remnic daemon URL, session key prefix, and connector auth token. Remnic writes it with owner-only permissions where the platform supports file modes.
...
| `sessionKeyPrefix` | `pi` | Session key prefix used when building session keys (e.g. `myuser:abc123`). Instead of hard-coding `pi:`, set this to your preferred principal identifier so the Remnic daemon can resolve it into the correct namespace and authorization rules. When unset, the daemon resolves namespace from the session key and auth principal. |
| `namespace` | unset | Optional explicit namespace override. When set, the daemon uses this namespace directly (bypassing coding-context and principal-based resolution). When unset, the daemon resolves the namespace from the session key, auth principal, and coding context. |
...
Example with explicit namespace (opts out of server-side resolution):
...
- Remnic routes Pi traffic through the daemon's namespace resolution (coding context, auth principal), NOT a client-side namespace override.
- The `sessionKeyPrefix` config lets administrators set a principal identifier (e.g. `"myuser"`) that the daemon maps to namespace and authorization rules.
